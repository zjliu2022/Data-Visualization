  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "page2",
              ...
              p('Please only select one of the following checkbox!'),
              checkboxInput("Monthly_Sales_1", label = "Rank by Monthly Sales", value = FALSE),
              checkboxInput("avg_price_rmb", label = "Rank by Price", value = FALSE),
              checkboxInput("comment_number", label = "Rank by comment number", value = FALSE),
              plotOutput("plot1")
      ),
      ...
      tabItem(tabName = "page3",
              ...
              p("Please only select one of the following checkbox!"),
              checkboxInput("Monthly_Sales", label = "Show Monthly_Sales Density(Orange)", value = FALSE),
              checkboxInput("Avg_Price", label = "Show Avg_Price Density(Pink)", value = FALSE),
              checkboxInput("Score", label = "Show Score Density(Green)", value = FALSE),
              checkboxInput("Count", label = "Show Merchant Count Density(Red)", value = FALSE),
...
server <- function(input, output, session) {
  ...
  output$plot1 = renderPlot({
    ...
    if(input$Monthly_Sales_1 == TRUE) {
      data = data_map %>% 
        arrange(desc(month_sales_clean)) %>% 
        slice(1:10) 
      plot = data %>%
        ggplot(aes(x=name, y=month_sales_clean, fill=name)) +
